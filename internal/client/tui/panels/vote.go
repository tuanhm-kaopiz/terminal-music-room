...
func Vote(tm theme.Theme, v state.View, width int) string {
	return strings.Join(voteLines(tm, v, width), "\n")
}
...
func Signals(tm theme.Theme, v state.View, width, height int, opts RenderOpts) string {
	lines := []string{tm.Title().Render("SIGNALS")}
	lines = append(lines, voteLines(tm, v, width)...)
	lines = append(lines, reactionLines(tm, v, width)...)
	return wrapPanel(tm, opts.Focused, width, height, lines)
}

func voteLines(tm theme.Theme, v state.View, width int) []string {
	vote := v.Room.Vote
	if vote == nil {
		return []string{tm.Muted().Render("no active vote")}
	}
	...
	return []string{
		tm.Warning().Render(truncate(line, max(1, width-4))),
		bar,
	}
}
